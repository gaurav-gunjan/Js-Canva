                                ...
                                innerListItem.onclick = function () {
                                    ...
                                    if (category?._id) {
                                        ...
                                        fetch('http://192.168.1.60:5000/api/user/adminChildCategories', childOptions)
                                            .then(response => response.json())
                                            .then(resData => {
                                                console.log("Child Response Data ::: ", resData?.result)
                                                ...
                                                resData?.result?.map(childValue => {
                                                    ...
                                                    // var framesGrid = document.querySelector('.palleon-frames-grid');
                                                    innerDivElement.addEventListener('click', function (event) {
                                                        console.log("Inner Div Element")
                                                        console.log("frame", event.target.classList.contains('palleon-frame'))
                                                        console.log("Image", event.target.tagName === 'IMG')
                                                        console.log("Event ::: ", event.target.classList)

                                                        if (event.target.classList.contains('palleon-frame') || event.target.tagName === 'IMG') {
                                                            console.log("Inside Inner Div Element")
                                                            var loader = document.getElementById('palleon-canvas-loader');
                                                            loader.style.display = 'flex';

                                                            var frame = event.target.closest('.palleon-frame');
                                                            var svgUrl = frame.getAttribute('data-elsource');
                                                            console.log("Svg URL ::: ", svgUrl)

                                                            var frames = document.querySelectorAll('.palleon-frames-grid .palleon-frame');
